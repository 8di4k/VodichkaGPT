...
🎨 Generate images from text in <b>👩‍🎨 Artist</b> /mode
...
async def subscribe_handle(update: Update, context: CallbackContext):
    ...
    description = "Your subscription to our service"
    ...
    price = 29900  # For example, 100.00 RUB should be written as 10000
...
async def successful_payment_callback(update: Update, context: CallbackContext):
    # Payment is successful, so update the user's subscription
    user_id = update.message.chat.id
    # This example adds 30 days to the subscription
    end_date = datetime.now() + timedelta(days=30)
    db.update_user_subscription(user_id, end_date)

    await update.message.reply_text("Thank you for your payment. Your subscription has been activated!")
...
async def message_handle(update: Update, context: CallbackContext, message=None, use_new_dialog_timeout=True):
    ...
    if subscription is None or subscription['end_date'] < datetime.now():
        await update.message.reply_text("You need to have an active subscription to use this bot. Use /subscribe to get started.")
        return
...
async def show_balance_handle(update: Update, context: CallbackContext):
    ...
    if subscription_info:
        ...
        if days_left > 0:
            subscription_text = f"You have an active subscription with {days_left} days remaining."
        else:
            subscription_text = "Your subscription has expired. Please renew your subscription."
    else:
        subscription_text = "You do not have an active subscription."
